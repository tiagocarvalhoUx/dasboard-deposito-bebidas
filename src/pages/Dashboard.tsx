      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        <Card className="border-l-4 border-l-emerald-500 hover:shadow-lg transition-shadow overflow-hidden">
          ...
          <CardContent className="p-4 sm:p-6">
            ...
            <p className="text-xs sm:text-sm text-slate-600 mt-1 sm:mt-2 font-medium truncate">
        ...
        <Card className="border-l-4 border-l-amber-500 hover:shadow-lg transition-shadow overflow-hidden">
          ...
          <CardContent className="p-4 sm:p-6">
            ...
            <p className="text-xs text-slate-600 mt-1 sm:mt-2 font-medium truncate">
              Ticket médio:{" "}
              {metrics.vendasMes > 0
          ...
          <CardContent className="p-4 sm:p-6">
            <div className="space-y-2">
              {produtosBaixoEstoque.slice(0, 5).map((produto) => (
                <div
                  key={produto.id}
                  className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 bg-white rounded-lg border border-red-100"
                >
              ...
              {produtosBaixoEstoque.length > 5 && (
                <p className="text-xs sm:text-sm text-red-600 text-center pt-2">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium text-slate-500">
                      Nº
                    </th>
                    <th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium text-slate-500">
                      Cliente
                    </th>
                    <th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium text-slate-500 hidden md:table-cell">
                      Data
                    </th>
                    <th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium text-slate-500 hidden lg:table-cell">
                      Pagamento
                    </th>
                    <th className="text-right py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium text-slate-500">
                      Total
                    </th>
                    <th className="text-center py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium text-slate-500 hidden sm:table-cell">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {vendasRecentes.map((venda) => (
                    ...
                    >
                      ...
                      <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm truncate max-w-[120px] sm:max-w-none">
                        {venda.clienteNome}
                      </td>
                      ...
                      <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm text-right font-medium whitespace-nowrap">
                        {formatCurrency(venda.total)}
                      </td>
                      <td className="py-2 sm:py-3 px-2 sm:px-4 text-center hidden sm:table-cell">
                        <Badge
                          className={
                            getStatusBadgeClass(venda.status) + " text-xs"
                          }
                        >
